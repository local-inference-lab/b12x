...
CASES = (
    ...
    *(f"mhc:{op}:{hidden}:{capacity}:{variant}"
    ...
    "hyperconnection:grouped_rmsnorm", "hyperconnection:gate_mean", "moe:residency_updates", "moe:routing_profile_extent",
)
...
def declare(case):
    ...
    if case in ("moe:routing_profile", "moe:routing_profile_extent"):
        ...
        return op.plan_routing_profile(op.RoutingProfileQuery(layers=(("compile", 384),),
            ...
            runtime_token_limit=case.endswith("_extent")))
