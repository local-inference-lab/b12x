...
class TensorParallelResidencyMaintenance:
    ...
    def __init__(self, config, world_size):
        ...
        if (
            config.get("anchor_thresholds")
            or config.get("movement_mode", "adapt") != "adapt"
        ):
            raise ValueError(
                "TP maintenance requires ordinary adaptation without anchor recovery"
            )
    ...
    def run(self, collective_rpc):
        ...
        def rpc(stage, *args):
            ...
            try:
                return self._ranks(
                    collective_rpc("b12x_residency_" + stage, args=args, timeout=60)
                )
            finally:
        ...
        try:
            ...
            if not baseline:
                for name, controller in self.coordinator.controllers.items():
                    ...
                    cold += sum(
                        c
                        for c, (tier, _) in zip(
                            counts, slots[name].expert_map, strict=True
                        )
                        if tier
                    )
                decision = self.coordinator.observe(
                    snapshot,
                    slots=slots,
                    allow_movement=bool(selections)
                    and (self.threshold is None or cold / selections >= self.threshold),
                )
            pairs = (
                {n: () for n in slots}
                if baseline
                else {v.layer: v.pairs for v in decision.layers}
            )
            commands = {
                str(rank): {
                    n: {"expected": v["slots"], "pairs": pairs[n]}
                    for n, v in report["layers"].items()
                }
                for rank, report in ranks.items()
            }
            for stage, args, key in (
                ("prepare", (token, commands), "ready"),
                ("stage", (token,), "staged"),
            ):
            ...
            for rank, report in applied.items():
                ...
                for name, value in report["layers"].items():
                    ...
                    expected = ResidencySlotSnapshot(
                        ...
                        generation=before.generation + bool(pairs[name]),
                        healthy=True,
                        expert_map=updated_slot_map(
                            before,
                            pairs[name],
                            backing_mode=owner["layers"][name]["exchange"][
                                "backing_mode"
                            ],
                        ),
                    )
                    if _slots(value) != expected:
                        raise RuntimeError(
                            "TP published map differs from logical decision"
                        )
            if not all(
                v.get("acknowledged") is True
                for v in rpc("acknowledge_staged", token).values()
            ):
                raise RuntimeError("TP publication acknowledgement missing")
            outcomes = (
                {}
                if baseline
                else self.coordinator.finish(
                    decision,
                    slots={
                        n: _slots(v)
                        for n, v in applied[self.owner_rank]["layers"].items()
                    },
                )
            )
            if not all(
                v.get("finished") is True for v in rpc("finish", token).values()
            ):
            ...
            if not publication:
                try:
                    ...
                    self.last_receipt["rollback_error"] = (
