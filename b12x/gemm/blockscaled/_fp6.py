...
def execute(lhs, rhs, out, *, ab_dtype, sf_dtype, sf_vec_size, c_dtype,
            ...
            b_packed=False, alpha=None, row_scale=None, stream=None):
    ...
    if m:
        _execute(a, b, sfa, sfb, out, alpha, row_scale, a_fmt, b_fmt,
                 a_bytes, b_bytes, c_dtype, cuda_stream_to_int(stream))
    return out
...
def _execute(a: torch.Tensor, b: torch.Tensor, sfa: torch.Tensor, sfb: torch.Tensor,
    ...
    with torch.cuda.device(a.device):
        ...
        fn(*(pointer(dtype, tensor) for dtype, tensor in zip(dtypes, (a, b, sfa, sfb, out, alpha, row_scale), strict=True)),
...
def _execute_fake(a, b, sfa, sfb, out, alpha, row_scale, a_fmt, b_fmt,
                  a_bytes, b_bytes, c_dtype, stream_int):
    return None
