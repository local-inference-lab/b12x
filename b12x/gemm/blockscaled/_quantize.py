...
def _quantize(
    ...
    if FP4:
        if GROUP == 32:
            ...
            sf_byte = tl.minimum(exponent + bump.to(tl.uint32), 254).to(tl.uint8)
            sf_byte = tl.where(ratio <= 0, 0, sf_byte).to(tl.uint8)
            inverse = ((254 - sf_byte.to(tl.uint32)) << 23).to(tl.float32, bitcast=True)
            inverse = tl.where(sf_byte == 0, 0.0, inverse)
        else:
            g = tl.load(AG)
        ...
        if GROUP == 32:
            code |= ((x.to(tl.uint32, bitcast=True) >> 28) & 8).to(tl.uint8)
        else:
        ...
        if (row == 0) & (tl.program_id(1) == 0):
            tl.store(ALPHA, 1.0)
    scale_groups: tl.constexpr = triton.cdiv(K // GROUP, 4) * 4 if FP4 and GROUP == 32 else K // GROUP
