...
class K6McgSmallMKernel:
    ...
    def _output_rotation_tile(
        ...
        # reloading the intermediate FP16 output from global memory.
        ...
        while row < active_m:
            ...
            v0, v1 = half2_to_float2_scaled(packed01, cutlass.Float32(1.0))
            v2, v3 = half2_to_float2_scaled(packed23, cutlass.Float32(1.0))
...
def compile_k6_mcg_small_m(
    *, size_k: int, size_n: int, device: torch.device
) -> K6McgSmallMCompileResult:
    """Compile the cooperative K6/MCG specialization for one K/N shape."""
    kernel = K6McgSmallMKernel(size_k=size_k, size_n=size_n)
    ...
    if resources is not None:
        ...
        if local_bytes != 0:
            raise RuntimeError(
                "K6/MCG small-row code generation spills to local memory "
                f"({local_bytes} bytes per thread)"
            )
    result = K6McgSmallMCompileResult(
        ...
        local_memory_bytes=int(local_bytes),
    )
...
def run_k6_mcg_small_m(
    ...
    *,
    ...
    launch.compiled(
        ...
        _trellis256_execution_lut(x.device, "mcg"),
        m,
        launch_grid_x,
        launch.grid_x,
        current_cuda_stream(),
