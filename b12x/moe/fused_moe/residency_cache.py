...
from .automatic import PHASES, ResidencyLayerSpec, RoutingSnapshot
from ._residency_updates import ResidencySlotSnapshot
...
class ResidencyCacheController:
    ...
    """
    def __init__(self, *, spec: ResidencyLayerSpec, config: ResidencyCacheConfig,
                 counter_query: RoutingProfileQuery, slots: ResidencySlotSnapshot,
        ...
        if (counter_query.rank != counter_query.owner_rank
                or dict(counter_query.layers).get(spec.layer) != spec.experts
        ...
        _validate_slots(slots, spec.experts)
        if not isinstance(baseline, RoutingSnapshot):
            raise TypeError("cache baseline requires RoutingSnapshot")
        self.spec, self.config, self.counter_query = spec, config, counter_query
        self._slots = slots
    ...
    def _row(self, snapshot):
        ...
        if snapshot.rank != self.counter_query.owner_rank or snapshot.epoch != self._epoch:
            raise ValueError("counter owner/reset epoch changed; establish a fresh policy baseline")
        matches = [row for row in snapshot.layers if row.layer == self.spec.layer and row.phase == self.config.phase]
        if len(matches) != 1 or len(matches[0].counts) != self.spec.experts:
    ...
    def observe(self, snapshot: RoutingSnapshot, *, slots: ResidencySlotSnapshot):
        ...
        if (sampled_calls > calls or sampled_tokens > tokens
                or sum(counts) > sampled_tokens*self.counter_query.max_top_k
        ...
        decision = ResidencyCacheDecision(window=window, expected=slots, pairs=tuple(pairs), counts=counts,
            calls=calls, sampled_calls=sampled_calls, sample_every=self.counter_query.sample_every,
    ...
    def finish(self, decision: ResidencyCacheDecision, *, slots: ResidencySlotSnapshot):
        ...
        _validate_slots(slots, self.spec.experts)
        pairs = ()
        if slots != self._slots:
        ...
        return ResidencyCacheOutcome(window=self._window, generation=slots.generation, pairs=pairs,
            ...
            evicted_promotion_hits=evicted_hits, committed_payload_copy_bytes=4*self.spec.expert_bytes*len(pairs),
            committed_map_copy_bytes=2*self.spec.experts*8 if pairs else 0)
