...
def route_pack_prewarm_key(
    ...
    counts can select different ``NUMEL_CAPACITY`` values. The caller-owned
    slot and block capacities are also part of the key because a fixed arena
    can replace the power-of-two bucket selected for the live shape.
    """
