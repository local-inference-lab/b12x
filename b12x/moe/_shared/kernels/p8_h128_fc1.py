...
class P8H128NarrowFC1Kernel(W4A8MaterializedPhase1Kernel):
    ...
    def _stage_slice(
        ...
        # published for them.
        for i in cutlass.range_constexpr(
            ((1 if self.p8_broadcast_a else self.tile_m) * 4 + self.threads_per_cta - 1) // self.threads_per_cta
        ):
            idx = tid + Int32(i * self.threads_per_cta)
    ...
    def _run_task(
        ...
        if cutlass.const_expr(self.p8_pipeline_stages == 3):
            self._stage_slice(
                ...
                token_map, smem_base, tid, source_m_tile, m_half,
                expert_idx, output_tile, valid_rows, Int32(1),
                input_k128_tiles, intermediate_tiles, packed_w13_tiles, subtile,
            )
            cute.arch.cp_async_commit_group()
        ...
        gate_acc = tuple(
            tuple(cute.make_rmem_tensor((4,), cutlass.Float32) for _nt in range(self.p8_n8_per_warp))
        ...
        if cutlass.const_expr(self.diagnostic_raw_fc1):
            ...
            if tid == Int32(0):
                ...
                # Localization receipt: prove the compiled epilogue width,
                # not merely the Python object's requested specialization.
        ...
        if cutlass.const_expr(self.full_coupled):
            ...
            for row_group in cutlass.range_constexpr(self.owned_row_groups):
                ...
                if hrow < valid_rows:
                    ...
                    g0, g1, g2, g3 = _w4a8_had128_quad(
                        g0, g1, g2, g3, lane
                    )
                    u0, u1, u2, u3 = _w4a8_had128_quad(
                        u0, u1, u2, u3, lane
                    )
                    local_col = output_tile * Int32(128) + hcol
                    g0 = self._scale_fc1_after_h128(
                        g0, trellis_rotations, expert_idx, local_col, Int32(0)
                    )
                    g1 = self._scale_fc1_after_h128(
                        g1, trellis_rotations, expert_idx, local_col + Int32(1), Int32(0)
                    )
                    g2 = self._scale_fc1_after_h128(
                        g2, trellis_rotations, expert_idx, local_col + Int32(2), Int32(0)
                    )
                    g3 = self._scale_fc1_after_h128(
                        g3, trellis_rotations, expert_idx, local_col + Int32(3), Int32(0)
                    )
                    u0 = self._scale_fc1_after_h128(
                        u0, trellis_rotations, expert_idx, local_col, Int32(1)
                    )
                    u1 = self._scale_fc1_after_h128(
                        u1, trellis_rotations, expert_idx, local_col + Int32(1), Int32(1)
                    )
                    u2 = self._scale_fc1_after_h128(
                        u2, trellis_rotations, expert_idx, local_col + Int32(2), Int32(1)
                    )
                    u3 = self._scale_fc1_after_h128(
                        u3, trellis_rotations, expert_idx, local_col + Int32(3), Int32(1)
                    )
                    # Luke stores the canonical gate/up boundaries as FP16
                    ...
                    a0 = self._scale_down_before_h128(
                        a0, trellis_rotations, expert_idx, local_col
                    )
                    a1 = self._scale_down_before_h128(
                        a1, trellis_rotations, expert_idx, local_col + Int32(1)
                    )
                    a2 = self._scale_down_before_h128(
                        a2, trellis_rotations, expert_idx, local_col + Int32(2)
                    )
                    a3 = self._scale_down_before_h128(
                        a3, trellis_rotations, expert_idx, local_col + Int32(3)
                    )
                    a0, a1, a2, a3 = _w4a8_had128_quad(
                        a0, a1, a2, a3, lane
                    )
                    st_shared_u32(goff, _p8_pack_f32x2_to_half2(a0, a1))
                    st_shared_u32(
                        goff + Int32(4), _p8_pack_f32x2_to_half2(a2, a3)
                    )
        ...
        if cutlass.const_expr(self.p8_warp_quant):
            # One warp per row, four independent eight-lane K32 groups.
            # Each lane stores one packed word in the existing trellis order.
            block = lane >> Int32(3)
            word = lane & Int32(7)
            for row_group in cutlass.range_constexpr(self.owned_row_groups):
                ...
                if row < valid_rows:
                    ...
                    for elem in cutlass.range_constexpr(4):
                        ...
                        # Exact _W4A8_TRELLIS_A_K32_PERM bit permutation.
                        pi = ((i & Int32(1)) | ((i >> Int32(2)) & Int32(2))
                              | ((i & Int32(4)) ^ ((i >> Int32(2)) & Int32(4)))
                              | ((i & Int32(2)) << Int32(2)) | (i & Int32(16)))
                        value = ld_shared_f32(full_output_base +
                            (row * Int32(128) + block * Int32(32) + pi) * Int32(4))
                        values4[elem] = value
                    ...
                    payload = cvt_f32x4_to_e4m3x4(
                        values4[0] * inv_scale, values4[1] * inv_scale,
                        values4[2] * inv_scale, values4[3] * inv_scale)
                    dst = ((physical_row_base + row) * words_per_row +
                           output_tile * Int32(32) + block * Int32(8) + word)
                    ...
                    if word == Int32(0):
                        ...
                        scale_bytes[(sf_base + output_tile * rows_capacity +
                                     physical_row_base + row) * Int32(4) + block] = cutlass.Uint8(scale_byte & Uint32(0xFF))
        elif tid < valid_rows:
            scale_word = Uint32(0)
