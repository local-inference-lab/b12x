    ...
    *,
    ...
    if manifest.rates.structure == RATE_STRUCTURE_PER_EXPERT_PAIR:
        ...
        if supports_architecture(torch.cuda.get_device_capability(device), ("sm103a",)):
            ...
            return prepare_exl3_atom_weights(
    ...
    if manifest.hadamard.intermediate_hadamard:
        ...
        # The intermediate-Hadamard residual transform interleaves gate/up into
        # one length-2I axis whose two stored halves each carry one input-side
        # table. An extent may cross the halves; both physical FC1 slots then
        # use the same column-wise split.
        gate_suh, up_suh, input_scale_split = _intermediate_hadamard_input_tables(
            layer, gate_suh, up_suh
        )
    ...
    if manifest.rates.structure == RATE_STRUCTURE_UNIFORM:
        ...
        if tile_config is None:
            tile_config = (
                (128, 128, 128, 128)
                if manifest.hadamard.intermediate_hadamard and (
...
def _intermediate_hadamard_input_tables(layer, gate_suh, up_suh):
    pre_half_slots = layer.manifest.geometry.num_slots // 2
    if layer.first_slot < pre_half_slots < layer.first_slot + layer.slot_count:
        split = (pre_half_slots - layer.first_slot) * layer.manifest.geometry.slot_channels
