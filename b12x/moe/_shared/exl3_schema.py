...
class Exl3Manifest:
    ...
    def partition_extents(self, world_size: int) -> tuple[tuple[int, int], ...]:
        ...
        _require(
            ...
            "EXL3 world_size must be a positive integer",
        )
        alignment = self.layout.extent_alignment_slots
        boundaries = (0, *self.layout.extent_barriers, self.geometry.num_slots)
        units = tuple(
            (end - start) // alignment
        ...
        _require(
            ...
            "EXL3 barriers must align to complete rank extent units",
        )
        _require(
            len(units) <= world_size <= sum(units),
            "EXL3 world_size cannot cover all barriers with nonempty aligned extents",
        )
        ranks = [1] * len(units)
