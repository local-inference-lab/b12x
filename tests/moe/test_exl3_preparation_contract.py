"""Canonical EXL3 declarations reject incompatible metadata before CUDA work."""
...
def checkpoint(tmp_path_factory):
    raw, layer, _ = exl3_atom_fixture(tmp_path_factory.mktemp("exl3-public"))
...
def test_canonical_exl3_preserves_native_plan(checkpoint):
...
def test_exl3_geometry_mismatch(checkpoint, field, value):
...
def test_exl3_requires_a16_and_native_packing(checkpoint):
    _, _, source, activation, geometry = checkpoint
...
def test_exl3_bundle_rejects_wrong_source_or_destination(checkpoint):
    _, layer, source, activation, geometry = checkpoint
    plan = fused_moe.plan_weights(
        source=source, activation=activation, geometry=geometry
    )
    with pytest.raises(TypeError, match="Exl3Manifest"):
        fused_moe.Exl3Source(manifest={})
    with pytest.raises(TypeError, match="Exl3Layer"):
        fused_moe.Exl3Weights(layer=None, device="cuda")
    with pytest.raises(ValueError, match="CUDA destination"):
        fused_moe.Exl3Weights(layer=layer, device="cpu")
    with pytest.raises(TypeError, match="Exl3Weights"):
    ...
    with pytest.raises(ValueError, match="manifest differs"):
        fused_moe.prepare_weights(
            plan=plan, weights=fused_moe.Exl3Weights(layer=different, device="cuda")
        )
