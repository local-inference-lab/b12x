...
class _DecodeProbe:
    def __init__(self, shared_lut=False, selector_lut=False):
        self.shared_lut = shared_lut
        self.selector_lut = selector_lut
        self.table_bytes = IQ2_XS_SELECTOR_LUT_BYTES if selector_lut else IQ2_XS_MAGNITUDE_LUT_BYTES

    @property
    def __cache_key__(self):
        return (self.shared_lut, self.selector_lut)
    ...
    def kernel(self, base: cutlass.Int32, lut: cute.Tensor, out: cute.Tensor):
        ...
        if cutlass.const_expr(self.shared_lut):
            ...
            for i in cutlass.range_constexpr(self.table_bytes // (256 * 16)):
                cp_async4_shared_global(
...
def test_all_descriptors_subscales_and_rounding(base_bits, shared_lut, selector_lut):
    device = require_b12x()
    table = iq2_xs_execution_lut(device, prepare=True, selectors=selector_lut)
    ...
    compiled = b12x_compile(
        _DecodeProbe(shared_lut, selector_lut),
