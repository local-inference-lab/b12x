...
def test_trellis_launch_planner_compiles_fixed_launch_matrix() -> None:
    ...
    assert tuple(broadcast for broadcast, _launch in plan._prewarmed_fused_launches) == (
        False,
        True,
    )
    assert {
        (ids_dtype, mapped, broadcast)
        for ids_dtype, mapped, broadcast, _launch in plan._prewarmed_topk_sum_launches
    } == {
        (dtype, mapped, broadcast)
...
def test_trellis_binding_retains_capacity_launches_for_transform_layout(
    monkeypatch: pytest.MonkeyPatch,
    broadcast: bool,
) -> None:
    ...
    payload = SimpleNamespace(
        gate_suh=torch.empty(128 if broadcast else (8, 128)),
        down_svh=torch.empty(128 if broadcast else (8, 128)),
    )
    plan.bind(
        scratch=_scratch_for_plan(plan),
