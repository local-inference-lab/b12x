    ...
    try:
        ...
        for dtype in (torch.bfloat16,):
            for rows in (8, 64, 256):
                ...
                # A later default-output collective must not mutate a result
                # whose caller still retains it.
                for retained, retained_ref in zip(
                    retained_outputs, retained_refs, strict=True
                ):
                    _assert_close(retained, retained_ref, world_size)

        # Graph capture and replay with changing inputs. Captured callers
        # provide stable output storage explicitly.
        rows = 256
