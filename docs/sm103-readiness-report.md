...
The residency policy review evidence binds package SHA256
`6b1c98b0af787cfd0900d728732e9724c6a71bc4c4e2c57086355d00127f7f09`.
The [engineering ledger](expert-residency-ledger.md#residency-policy-review-evidence) records
commands, toolchains, device identity and receipt locations.

| Gate | Result and scope |
| --- | --- |
| Host preparation, architecture and selected MoE suites | 1,001 passed, 225 skipped, including rebased QSA/MLA host cases. Skipped physical-target cases are not counted as passes. |
| Prepared counter and worker lifecycle GPU gates | 6 passed and 6 SM103-only skips under each of memcheck and synccheck on SM120, zero errors; counters, lifecycle and device-batched timing checks. |
| Portable arithmetic and rebased attention | 8 residency arithmetic tests and 49 QSA/sparse-MLA tests passed on SM120. Native SM103 expert MMA remains unexecuted. |
| Historical portable profiler diagnostic | The preceding package `aac5c587…` measured off/on/sampled partition stages for M=1 through 128 and top-k=1/6/8, plus contention. Those measurements motivate temporary profiling but are not attributed to the rebased source or to B300/full-MoE performance. |
...
GitHub reports zero commit statuses and zero check runs for the reviewed starting source `2de9d31c` as of
