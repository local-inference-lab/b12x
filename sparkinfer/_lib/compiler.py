...
def _compile_memory_cache_key(
    ...
    if compile_spec is not None:
        ...
        key: tuple[object, ...] = (
            "sparkinfer_cute_memory_cache_v2_explicit_spec",
...
def _cute_compile_log_enabled() -> bool:
    raw = os.environ.get("SPARKINFER_LOG_CUTE_COMPILES", "")
...
def _cute_compile_stack_log_enabled() -> bool:
    raw = os.environ.get("SPARKINFER_LOG_CUTE_COMPILE_STACK", "")
    if raw:
        return raw.lower() not in {"0", "false", "no", "off"}
    raw = os.environ.get("SPARKINFER_LOG_CUTE_COMPILES", "")
...
def _cute_compile_stack_log_depth() -> int:
    raw = os.environ.get("SPARKINFER_LOG_CUTE_COMPILE_STACK_DEPTH", "")
    if not raw:
        return 48
    try:
        return max(1, int(raw))
...
def _compile_cache_payload_log_value(
    ...
    if len(payload) == 8 and payload[0] in {
        "sparkinfer_cute_compile_cache_v3_explicit_spec",
        "sparkinfer_cute_compile_cache_v4_explicit_spec",
    }:
        (
            _version,
            target_key,
...
def _cute_compile_arg_log_enabled() -> bool:
    raw = os.environ.get("SPARKINFER_LOG_CUTE_COMPILE_ARGS", "")
    return raw.lower() not in {"", "0", "false", "no", "off"}
...
def _format_cute_compile_stack() -> str:
    ...
    lines = ["[sparkinfer cute.compile] python_stack (most recent call last):"]
...
def _log_cute_compile_event(
    ...
    *,
    ...
    if _is_explicit_spec_payload(cache_payload):
        ...
        print(
            f"[sparkinfer cute.compile] {event} "
            f"{reason_text}"
...
def _call_cute_compile(
    ...
    *,
    ...
    print(
        f"[sparkinfer cute.compile] compile-start number={compile_number} {details}",
        flush=True,
    )
    ...
    try:
        ...
        print(
            f"[sparkinfer cute.compile] compile-failed number={compile_number} "
    ...
    with _COMPILE_PROGRESS_LOCK:
        ...
        total = _COMPILE_PROGRESS_TOTAL_SECONDS
    print(
        f"[sparkinfer cute.compile] compile-done number={compile_number} "
        f"duration_s={elapsed:.3f} total_compile_s={total:.3f} {details}",
        flush=True,
    )
    return compiled
...
@lru_cache(maxsize=1)
def _sparkinfer_package_fingerprint() -> str:
    return _compute_sparkinfer_package_fingerprint()


def sparkinfer_package_fingerprint() -> str:
    """Return the exact content fingerprint used by the CuTe object cache."""
    return _sparkinfer_package_fingerprint()
...
def _runtime_toolchain_key() -> tuple[object, ...]:
    ...
    return (
        ...
        ("sparkinfer_runtime_patches", cutlass_runtime_patch_status()),
    )
...
def _compile_environment_key() -> tuple[tuple[str, str], ...]:
    ...
    operational_env_vars = {
        ...
        "SPARKINFER_LOG_CUTE_COMPILE_STACK",
        "SPARKINFER_LOG_CUTE_COMPILE_STACK_DEPTH",
...
def _cache_prefix(cache_key: str) -> str:
    return f"sparkinfer_cute_{cache_key}"
