...
static int fused_threads() {
  static const int value = [] {
    int v = env_int("SPARKINFER_PCIE_FUSED_THREADS", 256);
...
static int fused_ctas_per_row_override() {
  static const int value = env_int("SPARKINFER_PCIE_FUSED_CTAS_PER_ROW", 0);
  return value;
}
...
static bool oneshot_push_enabled() {
  static const bool value = env_int("SPARKINFER_PCIE_ONESHOT_PUSH", 0) != 0;
  return value;
}
...
class PCIeAllreduce {
 public:
  ...
  void allreduce(cudaStream_t stream, T* input, T* output, int size, int threads = 256, int block_limit = 8) {
    ...
    static const int env_threads = env_int("SPARKINFER_PCIE_ONESHOT_THREADS", 0);
    static const int env_block_limit = env_int("SPARKINFER_PCIE_ONESHOT_BLOCK_LIMIT", 0);
