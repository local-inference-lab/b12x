...
def _wait_nanosleep_cycles_from_env() -> int:
    raw = os.getenv("SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES", "24")
    try:
        cycles = int(raw)
    except ValueError as exc:
        raise ValueError(
            "SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES must be an integer"
        ) from exc
    if not 0 <= cycles <= 1024:
        raise ValueError(
            "SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES must be in [0, 1024]"
        )
    return cycles
...
def _load_extension():
    ...
    return load(
        name=f"sparkinfer_pcie_vocab_argmax_ext_ns{wait_cycles}",
        sources=[str(source)],
        extra_cuda_cflags=[
            "-O3",
            f"-DSPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES={wait_cycles}",
        ],
        extra_ldflags=["-lcuda"],
        verbose=(os.getenv("SPARKINFER_PCIE_VOCAB_ARGMAX_VERBOSE_BUILD", "0") == "1"),
    )
