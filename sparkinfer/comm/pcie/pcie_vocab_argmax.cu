...
#ifndef SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES
#define SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES 24
#endif

static_assert(SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES >= 0);
...
__device__ __forceinline__ void wait_for(
  ...
  while (load_acquire(ptr) < generation) {
    ...
    SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES > 0
    __nanosleep(SPARKINFER_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES);
#endif
  }
}
