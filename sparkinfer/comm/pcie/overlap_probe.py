...
collective payloads. The probe uses SparkInfer's real PCIe DMA all-reduce and
...
* SparkInfer DMA versus NCCL for TP all-reduce payloads;
...
def dma_crossover_bytes(
    ...
    for index, (payload_bytes, decision) in enumerate(ordered):
        ...
        if len(tail) < consecutive_wins:
            break
        if decision.backend == "sparkinfer-dma" and all(
            item[1].backend == "sparkinfer-dma" for item in tail
        ):
