...
def parse_cuda_graph_dot(path: Path) -> list[KernelNode]:
    """Extract exact kernel symbols and launch attributes from a CUDA DOT graph."""
    ...
    for line_number, raw_line in enumerate(
        ...
        if id_match is not None:
            ...
            if launch_match is None:
                raise ValueError(f"{path}:{line_number}: unsupported CUDA launch")
            try:
                grid, block, dynamic_smem = _split_launch_geometry(
                    launch_match.group("geometry")
                )
                parsed_smem = _parse_dimension(dynamic_smem)
                if len(parsed_smem) != 1:
                    raise ValueError("dynamic shared-memory value is not scalar")
                current_launch = (
