    ...
    for mode, rows in (("decode", 1), ("decode", 8), ("extend", 64), ("extend", 1024)):
        ...
        plan, binding = prepare_compressed(session, caps,
            bind_args=dict(q=q, swa_indices=swa_indices, swa_lengths=swa_lengths,
                           indexed_indices=indices, indexed_lengths=lengths, indexed_page_table=table),
            run_args=dict(swa_k_cache=swa, indexed_k_cache=indexed, sm_scale=512 ** -0.5, out=out))
